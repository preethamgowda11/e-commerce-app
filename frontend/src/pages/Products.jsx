    ...
    <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'32px 16px' }}>
      ...
      <input placeholder="Search products..." value={search}
        ...
        style={{ width:'100%', padding:'10px 14px', border:'1px solid #ddd',
          borderRadius:'8px', fontSize:'14px', marginBottom:'28px' }} />
          ...
          {filtered.map(p => (
            <Link to={`/product/${p._id}`} key={p._id}
              ...
              <div style={{ background:'#fff', borderRadius:'12px', overflow:'hidden',
                boxShadow:'0 1px 6px rgba(0,0,0,0.08)', border:'1px solid #f0f0f0',
                transition:'transform 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.transform='translateY(-4px)'}
                onMouseLeave={e => e.currentTarget.style.transform='translateY(0)'}>
                <img
                  src={p.image || 'https://placehold.co/240x180?text=No+Image'}
                  alt={p.name}
                  style={{ width:'100%', height:'180px', objectFit:'cover' }} />
                <div style={{ padding:'14px' }}>
                  <p style={{ fontWeight:'600', fontSize:'15px', marginBottom:'4px' }}>{p.name}</p>
                  {p.category && (
                    <p style={{ fontSize:'12px', color:'#888', marginBottom:'8px' }}>{p.category}</p>
                  )}
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontWeight:'700', fontSize:'16px', color:'#4f46e5' }}>
                      ₹{p.price}
                    </span>
